...
|dotfiles-copyparty|Copyparty file server via `pipx`|Default shared paths: `$HOME/Downloads`, `$HOME/Pictures`, `$HOME/Videos`. Hosts without a systemd user session (WSL2 without systemd, or systemd-free distributions like Devuan/Artix) use a background-process fallback with state in `~/.local/state/copyparty/`|
